...
int main(int argc, char const* argv[]) {
    cxxopts::Options opts(
            "depinc-faucet", "Provide a service that can send amount to DePINC address with countable management.");
    opts.add_options()                      // All options here
            ("h,help", "Show help document")  // --help
            ("rpc-url", "Connect to depinc through this url",
             cxxopts::value<std::string>()->default_value("http://127.0.0.1:18732"))  // --rpc-url
            ("cookie-path", "The path to `.cookie`",
             cxxopts::value<std::string>()->default_value("$HOME/.depinc/testnet3/.cookie"))  // --cookie-path
            ...
            ("amount", "How many DePINC we should send to user on each request",
    ...
    PLOG_INFO << "Faucet for DePINC testnet3";
    ...
    Service service(
            ...
            [&rpc, amount, &addr_man, &db_path, secs_on_next_fund](
                ...
                PLOG_INFO << "Distribute fund " << amount << "DePINC to address `" << address << "`";
                try {
